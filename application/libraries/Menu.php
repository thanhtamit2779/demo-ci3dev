...
class Menu {
	...
	public function render_menu($name, $config = array(), $divided_items_list = array(), $divider = '')
	{
		$items = isset($this->menus[$name]) ? $this->menus[$name] : array();
		$this->items = $items;
		return $this->render($config, $divided_items_list, $divider);
	}
     ...
     */
    public function set_items($items = array(), $group = 'default')
    {
    	return $this->add_items($items, $group);
    	if(!$items) return '';
     ...
     * @param array $items data which would be rendered
     */
     public function add_items($items = array(), $group = 'default')
     {
     	if(!$items) return '';
    	// if(!$this->items) return $this->items = $items;
     	...
     	foreach($items  as $item)
     		$this->add_item($item, $group);
     }

     public function add_item($item = array(), $group = 'default')
     {
     	if(!$item) return '';
    	// if(!$this->items) return $this->items = $items;
     	if(!isset($this->menus[$group]))
    ...
    {	    
    	...
    	foreach ($items as $item)
    	{
    		...
    		if ( isset($item[$this->menu_label], $item[$this->menu_key]) ) 
    		{
    			...
    			$html .= $this->set_active($tag_open, $slug, $item);	        	        
	 ...
	 */
	private function set_active($html, $slug, $item = false)
	{
		$slug = trim($slug,'/');
		$slug = $slug.'/';
		...
		$base_url = preg_replace('/'.$prefix.'(\d+)'.$suffix.'/is','',  current_url());
		$base_url = preg_replace('/'.$prefix.'/i','',  $base_url);
		$base_url = preg_replace('/'.$suffix.'/i','',  $base_url);
		...
		if (!empty($item['is_active']) || ($this->item_active != '' && $slug == $this->item_active && empty($base_url)) || ($slug == $base_url)) 
		{
			$doc = new DOMDocument();
			$doc->loadHTML($html);
	...
	public function clear($group = 'default'){
		$this->items = array();
		$this->_additional_item = array();
		$this->divided_items_list = array();
		$this->item_active = '';
		$this->menus[$group] = array();
		return $this;
	}
}